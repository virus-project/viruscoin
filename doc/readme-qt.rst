VirusCoin-qt: Qt4 GUI for VirusCoin
===============================
...
Alternatively, install Qt Creator and open the `VirusCoin-qt.pro` file.

An executable named `VirusCoin-qt` will be built.
...
A warning for people using the *static binary* version of VirusCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of VirusCoin is linked against libdb5.3.
...
installed causes VirusCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
