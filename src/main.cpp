...
unsigned int nStakeMinAge = 7 * nOneDay; // 7 days as zero time weight
unsigned int nStakeMaxAge = 90 * nOneDay; // 90 dayss as full weight
...
const string strMessageMagic = "VirusCoin Signed Message:\n";
...
{
	if(nHeight == 1) return 77777 * COIN;
    if(nHeight >= 2 && nHeight <= 999) return 525 * COIN;
    if(nHeight >= 1000 && nHeight <= 1999) return 520 * COIN;
    if(nHeight >= 2000 && nHeight <= 2999) return 515 * COIN;
    if(nHeight >= 3000 && nHeight <= 3999) return 515 * COIN;
    if(nHeight >= 4000 && nHeight <= 4999) return 509 * COIN;
    if(nHeight >= 5000 && nHeight <= 5999) return 503 * COIN;
    if(nHeight >= 6000 && nHeight <= 6999) return 502 * COIN;
    if(nHeight >= 7000 && nHeight <= 7999) return 501 * COIN;
    if(nHeight >= 8000 && nHeight <= 8999) return 500 * COIN;
    if(nHeight >= 9000 && nHeight <= 10013) return 499 * COIN;
    ...
    // VirusCoin: subsidy is cut in half every 64x multiply of PoW difficulty
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nTime, bool bCoinYearOnly, int nHeight)
{
    ...
    if (Balance < (1111111 * COIN)) {
        nRewardCoinYear = 3;
    } else if (Balance >= (1111111 * COIN) &&  Balance <= (2222222 * COIN)) {
        nRewardCoinYear = 4;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("VirusCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in VirusCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // VirusCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VirusCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "VirusCoin2017";
        unsigned int nTimeGenesis = 1514067079;
        unsigned int nNonceGenesis = 393529;

        if (fTestNet)
        {
            pszTimestamp ="VirusCoin2017Test";
            nTimeGenesis = 1514067078;
            nNonceGenesis = 24707;
        }
        ...
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0x66306aa342c9495fe5ad4b9a3d0d4df028c33417d6c6889aa8def190865ce817"));
        else
            assert(block.hashMerkleRoot == uint256("0x2a47d5a86648e45a82390776953010128368cbba2f35dd6e0a8cc9ddabc38ab9"));
...
unsigned char pchMessageStart[4] = { 0xab, 0xde, 0x15, 0xa5 };
